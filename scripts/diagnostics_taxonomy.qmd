---
title: DASSIM deep sequencing taxonomy models diagnostics - Gaussian Process models
date: today
...
Plot traceplots and MCMC diagnostics for deep sequencing taxonomy models -
...
fitlistout <-
  read_rds(here("data_processed/gp_taxonomy_fitlistout.rda"))

mod_diagnostics_df <-
  read_rds(here("data_processed/gp_taxonomy_mod_diagnostics_df.rda"))

outcome_vars <- read_rds(here("data_processed/gp_taxonomy_outcome_vars.rda"))
```
...
for (i in seq_len(length(outcome_vars))) {
  plotlist[[i]] <-
    mcmc_trace(drawslistout[[i]], regex_pars = "alpha|beta|sigma|tau|length|phi") +
