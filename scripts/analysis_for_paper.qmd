...
btESBL_exposures |>
  ...
  arrange(desc(n)) |>
  filter(n > 0) |>
  mutate(perc = paste0(blantyreSepsis::sp_dc(100*n/N, 0),"%"),
...
mod_sum_df |>
  filter(grepl("beta\\[", parameter)) |>
  mutate(parameter = 
    case_when(grepl("1", parameter) ~ "Ceftriaxone",
  ...
  mutate(outcome_var = str_to_title(gsub("_", " ", outcome_var)),
         ...
         parameter = factor(
            parameter,
            levels = rev(c("Ceftriaxone", "Ciprofloxacin", "Cotrimoxazole",
