...
(p_fg1 / p_fg2) + plot_layout(heights = c(1.6, 1)) + plot_annotation(
  tag_levels =
    "A"
)
...
sf_a <-
  ...
  filter(!is.na(sample_type)) |>
  ggplot(aes(sample_type, Shannon,
...
left_join(
  read_tsv(here("data_raw/ECO_all_metagenomic_read_stats_for_JL_11_02_25.tab")) |>
    mutate(lab_id = str_extract(file, "C[0-9|A-Z]*(?=_)")) |>
    filter(lab_id != "C", !lab_id %in% samples_to_drop) |>
    group_by(lab_id) |>
    summarise(reads = sum(num_seqs)),
  read_tsv(here("data_raw/ECO_host_read_counts_08-10-25.txt")) |>
    mutate(lab_id = str_extract(Reads, "C[0-9|A-Z]*(?=_)")) |>
    filter(lab_id != "C", !lab_id %in% samples_to_drop) |>
    group_by(lab_id) |>
    summarise(human_reads = sum(Human_reads_removed)),
  by = join_by(lab_id == lab_id)
) |>
  mutate(prop = human_reads / reads) |>
  left_join(
    btESBL_stoolESBL |>
      select(lab_id, sample_type)
  ) |>
  ...
  summarise(
    median_reads = median(reads) / 1e6,
    lq_read = quantile(reads, 0.25) / 1e6,
    uq_read = quantile(reads, 0.75) / 1e6,
    mean_perc_human = median(prop) * 100,
    lq_perc = quantile(prop, 0.25) * 100,
    uq_perc = quantile(prop, 0.75) * 100
  ) |>
  kbl(caption = "Read statistics", col.names = c("Sample Type", rep(c(
    "Median",
    "LQ", "UQ"
  ), 2)), digits = 1) |>
  kable_classic(full_width = FALSE) |>
  add_header_above(c(" " = 1, "Number of reads (million)" = 3, "Percentage Human" = 3))
```
