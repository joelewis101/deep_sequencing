...
df2 <-
  ...
  left_join(
    btESBL_stoolESBL |>
      select(pid, lab_id, arm, visit, ESBL, enroll_date, data_date, sample_type) |>
...
sim_df <-
  bind_rows(
    ...
    sim_diversity(d, exposure = "amoxicillin", t_max = 100),
...
ggsave(here("plots/fig1.svg"), width = 11, height = 11)
```
...
ggsave(here("plots/fig2.svg"), width = 15, height = 17)
...
outcome_vars <-
 read_rds(here("data_processed/resfindermodel_outcome-vars.rda"))
...
mod_sum_df_bact <-
  read_rds(here("data_processed/resfindermodel_summary-df_bacteroidesbls-stratifed.rda"))
...
p_3f <-
  ...
  filter(grepl("beta\\[", parameter)) |>
  mutate(
    bacteroides_associated =
      if_else(grepl("_b$", outcome_var),
        "Yes", "No"
      ),
    bacteroides_associated = factor(bacteroides_associated,
      levels =
        c("Yes", "No")
    )
  ) |>
  ...
  mutate(
    outcome_var = gsub("_b", "", outcome_var),
    outcome_var = str_to_title(gsub("_", "-", outcome_var)),
    outcome_var = factor(outcome_var,
      levels = rev(c("Cephalosporin", "Carbapenem", "Beta-Lactam", "Methicillin"))
    )
  ) |>
  ggplot(aes(outcome_var, m,
    group = bacteroides_associated,
    color = bacteroides_associated,
    shape = bacteroides_associated,
    ymin = ll, ymax = hh
  )) +
  geom_point(position = position_dodge(width = 0.5)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  theme(legend.position = "bottom") +
  labs(x = "Gene subclass", y = "Parameter Value") +
  scale_color_brewer(palette = "Dark2", name = "Bacteroides\nAssociated") +
  scale_shape_manual(values = c(4, 16), name = "Bacteroides\nAssociated")


p_3a / p_3b / p_3c / (p_3d + p_3e) / p_3f + plot_layout(height = c(2.2, 1, 5, 1, 1)) + plot_annotation(tag_levels = "A")

ggsave(here("plots/fig3.pdf"), width = 16, height = 20)
ggsave(here("plots/fig3.svg"), width = 16, height = 20)
```
...
p_4a <-
  ...
  theme_bw() +
  theme(
    axis.text.x = element_text(size = 6, angle = 45, hjust = 1), legend.position =
      "none"
  ) +
...
ggsave(here("plots/fig4.pdf"), width = 15, height = 12)
ggsave(here("plots/fig4.svg"), width = 15, height = 12)
...
```{r}
#| label: fig-sfig1
#| fig-cap: "switches and co-administration of antimicrobials. (A) Proportion of participants receiving antibacterial or hospitalised as a function of time (only those antibiotics with prevalence in cohort > 5% shown); (B,C) Number of days of co-administration of antibacterial shown excluding (B) or including (C) cotrimoxazole. There were a total of 9,559 person-antibacterial days of antimicrobial exposure; of these there were 1,034/9,559 (11%) person-days of co-admimistration of cotrimoxazole with another antimicrobial and 384/9,559 (4%) person-days of co-administration of two non-cotrimoxazole antimicrobials."
#| fig.width: 11
#| fig.height: 8
...
ab_rank <-
  ...
  filter(pid %in% included_participants) |>
  rename(!!!rename_lookup) |>
  select(-c(assess_type, died, Hospitalised)) |>
  pivot_longer(-pid) |>
  group_by(pid, name) |>
  summarise(exposed = any(value == 1)) |>
  group_by(name) |>
...
sf_a <-
  ...
  group_by(assess_type, name) |>
  summarise(n = n(), x = sum(value == 1), prop = x / n) |>
  filter(name %in% c(
    ab_rank$name[1:7], "Hospitalised"
  )) |>
  filter(!name %in% c("TB therapy", "Fluconazole")) |>
  ungroup() |>
  mutate(
    name =
...
df_temp <-
  ...
  fill(everything()) |>
  ungroup() |>
  mutate(id = paste0(pid, "-", assess_type)) |>
  select(-c(pid, assess_type, died, `TB therapy`, Hospitalised)) |>
  pivot_longer(-id) |>
  filter(value == 1)


exc <-
  c("Aciclovir", "Fluconazole", "LA", "Quinine")
...
sf_b <-
  ...
  select(-c(id, value.x, value.y)) |>
  filter(!name.x %in% exc, !name.y %in% exc) |>
  filter(name.x != "Cotrimoxazole", name.y != "Cotrimoxazole") |>
  mutate(name.x = factor(name.x), name.y = factor(name.y)) |>
  group_by(name.x, name.y, .drop = FALSE) |>
  count() |>
  mutate(n = if_else(name.x == name.y, NA, n)) |>
  ggplot(aes(name.x, fct_rev(name.y), fill = n)) +
  geom_tile() +
  scale_x_discrete(position = "top") +
...
sf <-
  sf_a / (sf_b + sf_c) + plot_annotation(tag_levels = "A") + plot_layout(
    heights =
      c(1, 1.5)
  )
...
ggsave(here("plots/sfig01.svg"), sf, width = 11, height = 8)
ggsave(here("plots/sfig01.pdf"), sf, width = 11, height = 8)

# how many days of combination?
...
df_temp |>
  ...
  group_by(name.x, name.y, .drop = FALSE) |>
  count() |>
  mutate(n = if_else(name.x == name.y, 0, n)) |>
  ungroup() |>
  mutate(cat = if_else(name.x == "Cotrimoxazole" | name.y == "Cotrimoxazole", "cotrim", "other")) |>
  group_by(cat) |>
...
p_a <-
  mod_sum_df |>
  filter(!grepl("cef|hosp|cotrim|cip|amox|lp_|^eta|", parameter)) |>
  mutate(taxa = gsub("_.*$", "", outcome_var)) |>
  filter(taxa == "phylum") |>
...
ggsave(here("plots/sfig02.svg"), width = 8, height = 12)
ggsave(here("plots/sfig02.pdf"), width = 8, height = 12)
```
...
ggsave(here("plots/sfig03.pdf"), width = 10, height = 13)
ggsave(here("plots/sfig03.svg"), width = 10, height = 13)
```
...
ggsave(here("plots/sfig04.pdf"), width = 10, height = 6)
ggsave(here("plots/sfig04.svg"), width = 10, height = 6)
```
...
ggsave(here("plots/sfig05.pdf"), width = 13.5, height = 15)
ggsave(here("plots/sfig05.svg"), width = 13.5, height = 15)
```
...
mod_sum_df |>
  filter(grepl("sigma|alpha|length|tau|beta_0|beta_s", parameter)) |>
  mutate(
    outcome_var = str_to_title(gsub("_", " ", outcome_var)),
    # outcome_var = str_wrap(outcome_var, width = 25),
...
ggsave(here("plots/sfig06.pdf"), width = 14, height = 10)
ggsave(here("plots/sfig06.svg"), width = 14, height = 10)
```
...
ggsave(here("plots/sfig07.pdf"), height = 18, width = 14)
ggsave(here("plots/sfig07.svg"), height = 18, width = 14)
```
...
```{r}
#| label: fig-sfig8
#| fig-cap: "Distribution of ResFinderFG-identfied gene annotations (A) and prevalence of genes expected to confer resistance to antimicrobial agents (B) stratified by arm and visit. In comparison to the AMRFinderPlus analysis there is no apparent relationship between study visit/arm and prevalence. "
#| fig.width: 13
#| fig.height: 14
...
amr_fg |>
  group_by(class, name) |>
  summarise(value = if_else(any(value == 1), 1, 0)) |>
  left_join(    
    btESBL_stoolESBL |>
      transmute(name = tolower(lab_id),
arm = arm) |>
      unique(),
    by = join_by(name == name)) |>
  filter(!is.na(arm), value == 1) |>
  mutate(
    arm = case_when(
      arm == 1 ~ "Hospitalised\nAntibiotic",
      arm == 2 ~ "Hospitalised\nNo antibiotic",
    ...
    # subclass = str_to_title(class)
  ) |>
  unique() |>
  ggplot(aes(fct_infreq(class))) +
  geom_bar() +
  coord_flip() +
  facet_wrap(~ arm, ncol = 3, scale= "free_x") +
  theme_bw() +
  labs(x = "Gene annotation")

# amr_fg_matrix <-
# amr_fg |>
#     select(c(class, name, value)) |>
#     pivot_wider(id_cols = name, names_from =
#       class, values_from = value,
#       values_fn = sum) |>
#     mutate(across(where(is.numeric), \(x) if_else(x > 0, 1,0)))
#
#
# amr_fg_matrix <- amr_fg_matrix[, -1]
#
# amr_fg_matrix <- as.data.frame(amr_fg_matrix)
#
#
# library(pheatmap)
#
# pheatmap(amr_fg_matrix)
#
#
# gt <- pheatmap(amr_fg_matrix, filename="image.png")$gtable
# ggsave("image.pdf", plot=gt, width = 10, height = 10)
#
...
amr_fg |>
  left_join(    
    btESBL_stoolESBL |>
      transmute(name = tolower(lab_id),
  arm = arm, pid = pid, visit = visit) |>
      unique() ,
    by = join_by(name == name)) |>
  filter(!is.na(arm)) |>
  group_by(pid, arm, visit, subclass) |>
  summarise(value = if_else(any(value == 1), 1, 0)) |>
  group_by(arm, visit, subclass) |>
  summarise(
    N = length(value),
    n = sum(value)
  ) |>
  ...
  mutate(
    subclass = 
    str_to_title( recode(subclass, !!! resfinderFG_lookup))
) |>
  # filter(grepl("gent|aminoglyco|kanamycin_quionolone", class))  |>
  ggplot(aes(visit, prop, ymin = lci, ymax = uci, color = arm, group = arm)) +
  geom_point(alpha = 0.7) +
  geom_line(alpha = 0.7) +
  geom_errorbar(width = 0, alpha = 0.7) +
  facet_wrap(.~ subclass,ncol = 8, labeller = labeller(class = label_wrap_gen(25))) +
  theme_bw() +
  theme(strip.text = element_text(size = 7)) +
  scale_color_viridis_d() +
  labs(color = "", x = "Study Visit", y = "Prevalence") +
...
(p_fg1 / p_fg2) + plot_layout(heights = c(1.6,1)) + plot_annotation(tag_levels =
  "A")

ggsave(here("plots/sfig08.pdf"), height = 14, width = 13)
ggsave(here("plots/sfig08.svg"), height = 14, width = 13)
...
```{r}
#| label: fig-sfig9
#| fig-cap: "(A) Prevalence of ResFinderFG-identified resistance genes expected to confer resistance to given antimicrobial agents with binomial confidence intervals; (B) association of exposures (panels) with presence of resistance genes expected to confer resistance to given agents (y axis). Parameter values from beta parameters shown on x axis (see main text for details); error bars show 95% CrI. Parameter values for which 95% CrI do not cross 0 are highlighted in red."
#| fig.width: 10
#| fig.height: 10


mod_sum_df <- 
  read_rds(here("data_processed/resfinderFGmodel_summary-df.rda"))

mod_draws <-
  read_rds(here("data_processed/resfinderFGmodel_posterior-draws-list.rda"))

outcome_vars <-
 read_rds(here("data_processed/resfinderFGmodel_outcome-vars.rda"))
...
fact_levels <-
  ...
  mutate(
    outcome_var = 
    str_to_title(recode(outcome_var, !!! resfinderFG_lookup))
) |>
  mutate(outcome_var = str_to_title(gsub("_", " ", outcome_var))) |>
  arrange(desc(m)) |>
  pull(outcome_var)
...
amr_fg |>
  ...
  ggplot(aes(fct_reorder(subclass, prop), prop, ymin = lci, ymax = uci)) +
  geom_point(position = position_dodge(width = 0.6)) +
  geom_errorbar(width = 0, position = position_dodge(width = 0.6)) +
  coord_flip() +
  theme_bw() +
...
p_fg_prev / p_fg_mod + plot_annotation(tag_levels = "A")

ggsave(here("plots/sfig09.pdf"), width = 10, height = 10)
ggsave(here("plots/sfig09.svg"), width = 10, height = 10)


btESBL_exposures |>
  left_join(btESBL_participants |>
    select(pid, arm),
    by = "pid") |>
  filter(pid %in% included_participants, arm == 1) |>
  select(-c(assess_type, died, hosp, arm,
    tb, coart,
    quin, coart, fluco, ampho, arte,
    acicl
    )) |> 
  pivot_longer(-pid) |>
  group_by(pid, name) |>
...
ggsave(here("plots/sfig10.pdf"), width = 14, height = 24)
ggsave(here("plots/sfig10.svg"), width = 14, height = 24)
```
...
ggsave(here("plots/sfig11.pdf"), width = 12, height = 12)
ggsave(here("plots/sfig11.svg"), width = 12, height = 12)
```
...
ggsave(here("plots/sfig12.pdf"), width = 10, height = 6)
ggsave(here("plots/sfig12.svg"), width = 10, height = 6)
...
ggsave(here("plots/sfig13.pdf"), height = 18, width = 14)
ggsave(here("plots/sfig13.svg"), height = 18, width = 14)
```
...
```{r}
#| label: fig-sfig14
#| fig-cap: "Number of samples included in analysis stratified by study visit and study arm."
#| fig.width: 5
#| fig.height: 5

sf <-
otu_r_df |>
  select(pid,arm,visit) |>
  unique() |>
  mutate(arm = as.character(arm)) |>
  ggplot(aes(visit, fill = arm, group = arm)) +
  geom_bar() +
  theme_bw() +
  scale_fill_viridis_d() +
  labs(fill = "Study\nArm", x = "Study Visit", y = "Number of Samples")

sf

ggsave(here("plots/sfig14.pdf"), sf, width = 5, height = 5)
ggsave(here("plots/sfig14.svg"), sf, width = 5, height = 5)
```
## Supplementary Figure 15

```{r}
#| label: fig-sfig15
#| fig-cap: "Comparing rectal swab and stool samples. (A) Shannon (alpha) diversity; (B) principal coordinate plot of all-against-all Bray Curtis dissimilarity (beta diversity) with 95% confidence intervals assuming student T distribution stratified by sample type; (C) Proportion of reads assigned to a given phylum for each sample, ordered by proportion of Proteobacteria and stratified by sample type. Overall, rectal swab and stool samples are similar."
#| fig.width: 12
#| fig.height: 10
...
otu_r_df |>
  ...
  group_by(name, Phylum, sample_type) |>
  summarise(value = sum(value)) |>
  filter(Phylum != "") |>
  mutate(name = factor(name, levels = prot_rank_var$name)) |>
  filter(!is.na(sample_type)) |>
  ggplot(aes(name, value, fill = Phylum)) +
  geom_col() +
  facet_grid(~ sample_type, scales = "free_x", space = "free") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, size = 1)) +
  labs(x = "Sample ID", y = "Proportion of Reads")
...
estimate_richness(biom, measures = "Shannon") |>
  ...
  filter(!is.na(sample_type)) |>
  ggplot(aes(sample_type, Shannon, group = sample_type, color = sample_type,
      fill = sample_type)) + 
    geom_jitter(size = 0.5) +
  geom_boxplot(alpha = 0.5, outliers = FALSE) +
  theme_bw() +
  labs(y = "Shannon Diversity", x = "", fill = "Sample Type", color = "Sample Type") +
    theme(legend.position = "none")
...
sf_b <-
  ...
  filter(!is.na(sample_type)) |>
  ggplot(aes(Axis.1, Axis.2, color = sample_type, fill = sample_type, group = sample_type, shape = sample_type)) +
  geom_point(key_glyph = "polygon") +
  stat_ellipse(alpha = 0.1, geom = "polygon", show.legend = FALSE) +
  theme_bw() +
  # scale_color_viridis_d() +
  # scale_fill_viridis_d() +
  guides(fill = guide_legend(override.aes = list(alpha = 0.5))) +
  labs(
    x = "Axis 1 [25.6%]", y = "Axis 2 [14.8%]",
, fill = "Sample Type", color =
    "Sample Type", shape = "Sample Type"
  ) 

sf <-
(sf_a + sf_b + plot_layout(widths = c(0.5,1))) / sf_c +
  plot_annotation(tag_levels = "A")

sf

ggsave(here("plots/sfig15.pdf"), sf, width = 12, height = 10)
ggsave(here("plots/sfig15.svg"), sf, width = 12, height = 10)
...
# extras for manuscript


# subset_taxa(biom, Species == "sapiens")@otu_table@.Data -> human_reads
#
# biom@otu_table@.Data |>
#   as_tibble(rownames = "tax_id") |>
#   pivot_longer(-tax_id) |>
#   group_by(name) |>
#   summarise(reads = sum(value)) -> reads
#
#
# reads |>
#   left_join(
...
#   summarise(
#     mean_reads = mean(reads)/1e6,
#     sd_read = sd(reads)/1e6,
#     mean_prop_human = mean(prop),
#     sd_prop_human = sd(prop)
#   )
#
# readstats <- read_tsv(here("data_raw/ECO_all_metagenomic_read_stats_for_JL_11_02_25.tab"))
#
# data.frame(lab_id = dimnames(human_reads)[[2]], hum_reads = human_reads[1:426]) |>
#   left_join(
#   readstats |>
#   mutate(lab_id = str_extract(readstats$file, "C[0-9|A-Z]*(?=_)")) |>
#   filter(lab_id != "C", !lab_id %in% samples_to_drop) |>
#   group_by(lab_id) |>
#   summarise(reads = sum(num_seqs)),
#         by= join_by(lab_id == lab_id)) |>
#   mutate(prop = hum_reads/reads) |>
#   left_join(
#     btESBL_stoolESBL |>
#   select(lab_id, sample_type)) |>
#   group_by(sample_type) |>
#   summarise(
#     median_reads = mean(reads)/1e6,
#     lq_read = sd(reads)/1e6,
#     mean_prop_human = mean(prop)*100,
#     sd_prop_human = sd(prop)*100
#   )
#
