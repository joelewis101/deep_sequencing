...
df <-
  read_tsv(here("data_raw/resfinderFG/ECO_metagenomes_ResFinderFG_binary_matrix_v2.tsv")) |>
    clean_names() |>
    pivot_longer(-c(gene_symbol, class)) |>
    mutate(name = gsub("^sample_[0-9]*_","", name)) |>
    mutate(name = if_else(grepl("nc", name), gsub("...$", "", name), gsub("....$", "", name))) |>
    filter(!grepl("nc_", name), !name %in% tolower(samples_to_drop)) |>
    mutate(name =gsub("_", "", name)) |>
    select(-class) |>
    separate_wider_delim(gene_symbol, "|", names = c("class", "accession", "source", "subclass"))


df2 <-
df |>
   select(c(subclass, name, value)) |>
   pivot_wider(id_cols = name, names_from =
     subclass, values_from = value,
     values_fn = sum) |>
...
write_rds(mod_sum_df, 
  here("data_processed/resfinderFGmodel_summary-df.rda"), compress = "gz")

write_rds(fitlistout,
  here("data_processed/resfinderFGmodel_fitted-model-list.rda"), compress = "gz")

write_rds(drawslistout,
  here("data_processed/resfinderFGmodel_posterior-draws-list.rda"), compress = "gz")
...
write_rds(mod_diagnostics_df, 
  here("data_processed/resfinderFGmodel_diagnostics.rda"), compress = "gz")

write_rds(outcome_vars, here("data_processed/resfinderFGmodel_outcome-vars.rda"))
