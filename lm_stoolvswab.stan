...
matrix[n,n_covariates] t_e;                  // days since drug exposure; <0 =
                                             ...
                                             //col per drug                                             //
vector[n] stool;                      //  stool (1) or rectal swab(0)
...
real beta_s;        // effect of stool vs rectal swab
vector[n] eta;
}
...
for (i in 1:n_participants) {
  ...
  vector[N[i]] s;       // vector of stool vs swcb
  //print("t");
  //print(t[p:(p+N[i]-1)]);
  ...
  //print("X");
  //print(X);
  y[p:(p+N[i] -1)] ~ normal(beta_0 + f + X*beta + beta_s*s, sigma);
  p = p + N[i];
}
